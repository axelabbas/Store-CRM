...
async function checkBirthdaysAndNotify() {
  ...
  try {
    ...
    console.log(
      `Checking birthdays for today (${todayMonth}-${todayDay}) and 2 days from now (${twoDaysMonth}-${twoDaysDay})`
    );
    ...
    customers.forEach((customer) => {
      if (customer.birthday) {
        ...
        console.log(
          `Customer: ${customer.fullName}, Birthday: ${
            customer.birthday
          }, Parsed: ${birthday.getMonth() + 1}-${birthday.getDate()}`
        );
      }
    });
    ...
    const birthdaysToday = customers.filter((customer) => {
      ...
      console.log(
        `Checking ${customer.fullName}: ${birthMonth}-${birthDay} vs today ${todayMonth}-${todayDay}`
      );
  ...
  for (const customer of customers) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.log(
          `✅ Telegram notification sent successfully for ${customer.fullName} (${timeFrame})`
        );
      }
    } catch (error) {
